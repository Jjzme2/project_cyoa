...
export default function NewStoryPage() {
  ...
  async function generateShape() {
    ...
    try {
      ...
      const res = await fetch('/api/stories/narrative-shape', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ description: customShapeDescription.trim() }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Could not generate a shape')
      setCustomShape(data.shape)
      toast.success(`"${data.shape.name}" is ready.`)
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Could not generate a shape')
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (storyNarrativeMode === 'custom' && !customShape) {
      toast.error('Generate a custom shape first, or pick a different story mood.')
      return
    }
      ...
      const storyRes = await fetch('/api/stories', {
        ...
        body: JSON.stringify({
          ...
          economyEffects: goapEnabled && economyEffects.length > 0
            ...
            : undefined,
          narrativeMode: storyNarrativeMode === 'inherit' ? undefined : storyNarrativeMode,
          customNarrativeShape: storyNarrativeMode === 'custom' && customShape ? customShape : undefined,
      ...
      {selectedWorld && resolveNarrativeMode(selectedWorld) !== 'gentle' && (
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => setStoryNarrativeMode(e.target.value as typeof storyNarrativeMode)}
            className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          >
          ...
          {storyNarrativeMode === 'custom' && (
            <div className="space-y-2 pt-1">
              <Textarea
                value={customShapeDescription}
                onChange={(e) => { setCustomShapeDescription(e.target.value); setCustomShape(null) }}
                placeholder="e.g. a slow-burn heist where the crew starts as strangers and the real target is trust"
                rows={2}
              ...
              <Button
                ...
                disabled={generatingShape || !customShapeDescription.trim()}
                className="gap-2 text-xs"
              >
                {generatingShape ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                {generatingShape ? 'Shaping…' : 'Generate shape (1 credit)'}
              </Button>
              {customShape && (
                <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.06] p-3 space-y-1.5">
                  <p className="text-xs font-semibold text-amber-300">{customShape.name}</p>
                  <ol className="text-[11px] text-muted-foreground/60 space-y-0.5 list-decimal list-inside">
                    {customShape.beats.map((b, i) => <li key={i}>{b}</li>)}
                  </ol>
                </div>
              )}
            </div>
