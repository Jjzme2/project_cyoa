 ...
 */
export function endingDirective(depth: number, state: EngineState | undefined): string {
  if (depth < ENDING_MIN_DEPTH) return ''

  const beatIndex = state?.plot?.beatIndex ?? 0
  if (beatIndex >= RESOLUTION_BEAT) {
    return 'the central through-line has reached its resolution — bring the story to a close if this chapter can land it.'
  }
  if (depth >= SOFT_TARGET_DEPTH) {
    return 'this path has run long; watch for the earned moment to bring it home.'
  }
  ...
  if (tension < 0.25 && turnsSinceSpike >= 3) {
    return 'the storm has passed and the tension has settled — this may be the natural place to close.'
  }
